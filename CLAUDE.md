...
- `gh pr view --json statusCheckRollup` and `gh pr checks` with `--repo nightowlstudiollc/amelia-boone` fail with "Resource not accessible by personal access token" for status check rollup fields
- Use `gh api repos/nightowlstudiollc/amelia-boone/actions/runs` and `gh run view` instead to inspect CI status

### Code Formatting
*~900 tokens/session saved*
- Run `pnpm run format:check` before committing; CI enforces Prettier formatting and will fail if any file is not formatted
- Fix with `pnpm exec prettier --write <file>` (use `pnpm exec prettier`, not `npx prettier`, to avoid "no files matching pattern" errors when run from project root)

### Git Workflow
*~800 tokens/session saved*
- `gh pr merge` requires both `--squash` AND `--delete-branch` flags; omitting `--delete-branch` causes a pre-merge hook failure with message: "Non-interactive merge requires --squash and --delete-branch"
- Always use: `gh pr merge <num> --repo nightowlstudiollc/amelia-boone --squash --delete-branch`

### Branch Protection
*~600 tokens/session saved*
- Direct force-pushes to `main` are blocked by a pre-receive hook; bypass requires disabling the branch protection rule in GitHub settings first
- History rewriting (e.g., `git filter-repo`) must be followed by `git push --force --all --no-verify` after temporarily disabling branch protection

### Pre-commit Hooks
*~500 tokens/session saved*
- The repo uses a global git hooks config (`~/.config/git/hooks/pre-commit`) that runs Semgrep among other checks
- A `.semgrepignore` file can be created in the repo root to suppress Semgrep false positives during commit

### Repository
*~200 tokens/session saved*
- GitHub org: `nightowlstudiollc`, repo: `amelia-boone`; always pass `--repo nightowlstudiollc/amelia-boone` to `gh` commands
- Amelia Boone is an active client (not a demo/prospective client); this is a live publishing repo
